...
axis([-123.288 -123.268 44.567 44.575])         % View for Tyler
...
while 1
    ...
    lat = str2double(strip(char(dataMatrix(1))));   % Latitude
    lon = str2double(strip(char(dataMatrix(2))));   % Longitude
    vBatt = str2double(strip(char(dataMatrix(3)))); % Battery voltage
    temp = str2double(strip(char(dataMatrix(4))));  % Temperature
    analogRead1 = str2double(strip(char(dataMatrix(5))));   % AnalogRead1 value
    analogRead2 = str2double(strip(char(dataMatrix(6))));   % AnalogRead2 value
    packetnum = str2double(strip(char(dataMatrix(7))));     % Packet Number
    % Time stuff
    ...
    delete(h1); 
    h1 = plot(lon, lat, '.r', 'MarkerSize', 10);    % Plot point
    ...
    t1 = text(-123.281,44.5745,txt, 'Color', 'k', 'BackgroundColor', 'w');      % Text for Tyler 
end		% Loop Stop
hold off
% Closing Serial
fclose(s)
